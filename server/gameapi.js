...
DatabaseProxy.prototype.init = function() {
    ...
    this.password = process.env.OPENSHIFT_MYSQL_DB_PASSWORD ||  'secret';
    this.database = "nodegame"; // Make sure that the db name is correct (check MySQL code)
...
DatabaseProxy.prototype.createUser = function(websocket, msg) {
    ...
    this.connection.query('INSERT INTO userdata (username, password_hash) VALUES (?, ?)', [msg.username, msg.passwordhash], function(err, rows) {
        if (!err) {
...
DatabaseProxy.prototype.login = function(websocket, msg) {
    ...
    if (typeof this.gameAPI.players[msg.username] === 'undefined') {
        this.connection.query('SELECT password_hash FROM userdata WHERE username = ?', [msg.username],
            function(err, rows) {
                if (!err) {
